...
/* スレッド・コンテキスト */
typedef struct _kz_context { // コンテキスト保存用の構造体
  // 汎用レジスタはスタックに保存されるため保存しなくていい
  uint32 sp; /* スタック・ポインタ */
} kz_context;
...
typedef struct _kz_msgbox {
  ...
  kz_msgbuf *head; // メッセージキュー
  kz_msgbuf *tail;

  /*
   * 構造体の配列とかを作ると
   * 計算で乗算が使われて「___mulsi3が無い」などのリンク・エラーになる場合がある
   * (32bit乗算がない)
   * サイズを2の累乗にするとシフト命令が使われるためダミーを入れて調整
   */
  long dummy[1]; // サイズ調整用のダミー
} kz_msgbox;
...
void dispatch(kz_context *context); // 実体はstartup.sに
                                 ...
                                 int stacksize, int argc, char *argv[])
{
  ...
  *(--sp) = (uint32)thread_end;
  ...
  /*
   * プログラム・カウンタを設定する．
   * スレッドの優先度がゼロの場合には割込み禁止スレッドとする
   */
  // rte命令が呼ばれた時にPCに設定される p.247参考
  // CCRの割込み禁止フラグを立てる
  ...
  *(--sp) = 0; /* ER3 */
  *(--sp) = 0; /* ER2 */
  *(--sp) = 0; /* ER1 */ // この辺にargc,argvを引数として渡すこともできるが
                         // 別CPUでも同じ仕様とは限らないのでやらない
...
static int thread_exit(void)
{
   ...
   *     *(--sp) = (uint32)thread_end;のあたりで 実際にspが減っている
   * 解放を入れようと思うと，空いた領域が飛び飛びになったりで面倒そうなのはわかる
   */
  puts(current->name);
  puts(" EXIT.\n");
...
{
  putcurrent(); // レディーキューからいったん外して再push -> 後ろに回る
  return 0;
}
...
{
  return 0; // キューから外されたままになり回ってこなくなる
}
...
{
  putcurrent(); // 割込時に抜かれるので入れ直す
  return (kz_thread_id_t)current; // idを返す
}
...
static int thread_send(kz_msgbox_id_t id, int size, char *p)
{
  ...
  if (mboxp->receiver) {
    current = mboxp->receiver; /* 受信待ちスレッドをウェイクアップする */
...
static kz_thread_id_t thread_recv(kz_msgbox_id_t id, int *sizep, char **pp)
{
  ...
  if (mboxp->head == NULL) {
     ...
     */
    // putcurrent() を発行しなければ，recvを呼んだスレッドはキューに入らない（止まる）
    return -1;
  }
...
static int thread_setintr(softvec_type_t type, kz_handler_t handler)
{
  ...
  putcurrent(); // 処理後にレディーキューに接続しなおす
                // kz_start 内でも呼ばれるがNULLがcurrentはNULLで問題なし
